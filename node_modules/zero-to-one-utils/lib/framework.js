...
const fs = require('fs');
const debug = require('debug')('zero-to-one-utils:framework');
...
function getFrameworkPath({ framework, baseDir }) {
  ...
  //debug(pkg.egg.framework);
  // 2. framework is not specified
  // 2.1 use framework name from pkg.egg.framework
  // if (pkg.zero-to-one && pkg.zero-to-one.framework) {
  //   return assertAndReturn(pkg.egg.framework, moduleDir);
  // }

  // 2.2 use egg by default
  // debug(assertAndReturn('zero-to-one', moduleDir));
  return assertAndReturn('zero-to-one', moduleDir);
}
